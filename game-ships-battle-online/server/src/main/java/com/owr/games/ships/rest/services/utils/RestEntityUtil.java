...
public class RestEntityUtil {
    ...
    private static void setsPlayerEntity(ShipGamePlayerRestEntity playerEntity, PlayerEntity player, PlayerEntity playerTurn, String playerMap, String token  ) {
        boolean isItOwner = getNotNullToken(player).equalsIgnoreCase(token);
        BattleFieldSerialized bfSer = new BattleFieldSerialized(playerMap);

        playerEntity.setMap((isItOwner ? bfSer.getOriginalView() : bfSer.getMaskedView()));
